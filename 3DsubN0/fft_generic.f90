...
contains
  ...
  subroutine fft_init_arg(pencil)     ! allow to handle Z-pencil input
    ...
    if (format==PHYSICAL_IN_X) then
       ...
       call decomp_info_init(nx_global, ny_global, nz_global, ph)
  ...
  ! to init library to perform FFTs on a mesh of arbitrarily given size
  subroutine fft_init_general(pencil, nx,ny,nz)
    ...
    if (format==PHYSICAL_IN_X) then
       call decomp_info_init(nx/2+1, ny, nz, fft)
       call decomp_info_init(nx, ny, nz, ph)
    else if (format==PHYSICAL_IN_Z) then
       call decomp_info_init(nx, ny, nz/2+1, fft)
       call decomp_info_init(nx, ny, nz, ph)
    end if
