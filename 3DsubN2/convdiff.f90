...
if (ifilter.eq.1) then
   if (nclz.eq.0) then
      call filz(td3,ta3,di3,sz,vz,fiffz,fifz,ficz,fibz,fibbz,filaz,&
           fiz1z,fiz2z,zsize(1),zsize(2),zsize(3),1)
      call filz(te3,tb3,di3,sz,vz,fiffz,fifz,ficz,fibz,fibbz,filaz,&
           fiz1z,fiz2z,zsize(1),zsize(2),zsize(3),1)
      call filz(tf3,tc3,di3,sz,vz,fiffz,fifz,ficz,fibz,fibbz,filaz,&
           fiz1z,fiz2z,zsize(1),zsize(2),zsize(3),0)
   endif
   if ((nclz.eq.1).or.(nclz.eq.2)) then
      call filz(td3,ta3,di3,sz,vz,fiffzp,fifzp,ficzp,fibzp,fibbzp,&
           filazp,fiz1z,fiz2z,zsize(1),zsize(2),zsize(3),1)
      call filz(te3,tb3,di3,sz,vz,fiffzp,fifzp,ficzp,fibzp,fibbzp,&
           filazp,fiz1z,fiz2z,zsize(1),zsize(2),zsize(3),1)
      call filz(tf3,tc3,di3,sz,vz,fiffz,fifz,ficz,fibz,fibbz,filaz,&
           fiz1z,fiz2z,zsize(1),zsize(2),zsize(3),0)
   endif
...
!FILTER IN Y
if (ifilter.eq.1) then 
   if ((ncly==1).or.(nclz==2)) then
      call fily(tg2,td2,di2,sy,vy,fiffyp,fifyp,ficyp,&
           fibyp,fibbyp,filayp,fiz1y,fiz2y,ysize(1),ysize(2),ysize(3),1)
      call fily(th2,te2,di2,sy,vy,fiffy,fify,ficy,fiby,&
           fibby,filay,fiz1y,fiz2y,ysize(1),ysize(2),ysize(3),0)
      call fily(ti2,tf2,di2,sy,vy,fiffyp,fifyp,ficyp,fibyp,&
           fibbyp,filayp,fiz1y,fiz2y,ysize(1),ysize(2),ysize(3),1)
   else
      call fily(tg2,td2,di2,sy,vy,fiffy,fify,ficy,fiby,fibby,&
           filay,fiz1y,fiz2y,ysize(1),ysize(2),ysize(3),1)
      call fily(th2,te2,di2,sy,vy,fiffy,fify,ficy,fiby,fibby,&
           filay,fiz1y,fiz2y,ysize(1),ysize(2),ysize(3),0)
      call fily(ti2,tf2,di2,sy,vy,fiffy,fify,ficy,fiby,fibby,&
           filay,fiz1y,fiz2y,ysize(1),ysize(2),ysize(3),1)
   endif
else
   tg2(:,:,:)=td2(:,:,:)
...
!FILTER IN X
if (ifilter.eq.1) then
   call filx(tg1,td1,di1,sx,vx,fiffx,fifx,ficx,fibx,fibbx,filax,&
        fiz1x,fiz2x,xsize(1),xsize(2),xsize(3),0)
   call filx(th1,te1,di1,sx,vx,fiffx,fifx,ficx,fibx,fibbx,filax,&
        fiz1x,fiz2x,xsize(1),xsize(2),xsize(3),1)
   call filx(ti1,tf1,di1,sx,vx,fiffx,fifx,ficx,fibx,fibbx,filax,&
        fiz1x,fiz2x,xsize(1),xsize(2),xsize(3),1)
else
   tg1(:,:,:)=td1(:,:,:)
   th1(:,:,:)=te1(:,:,:)
...
enddo
!call deryS (tb2,ta2,di2,sy,ffy,fsy,fwy,ppy,ysize(1),ysize(2),ysize(3),0)
if (istret.ne.0) then 
   !call deryyS (ta2,phi2,di2,sy,sfyp,ssyp,swyp,ysize(1),ysize(2),ysize(3),1)
   !call deryS (tc2,phi2,di2,sy,ffy,fsy,fwy,ppy,ysize(1),ysize(2),ysize(3),0)
   do k=1,ysize(3)
   do j=1,ysize(2)
   do i=1,ysize(1)
...
!!!!NEW NEW NEW
do ijk=1,nvect1
   ta1(ijk,1,1)=ta1(ijk,1,1)-(1./16.)*uy1(ijk,1,1)
enddo
...
!   do j=1,xsize(2)
!   do i=1,xsize(1)
!      phi1(i,j,k)=gdt(itr)*ta1(i,j,k)+phi1(i,j,k)
!      phis1(i,j,k)=ta1(i,j,k)
!   enddo
!   enddo
!   enddo
!else
...
!   do j=1,xsize(2)
!   do i=1,xsize(1)
!      phi1(i,j,k)=adt(itr)*ta1(i,j,k)+bdt(itr)*phis1(i,j,k)+phi1(i,j,k)
!      phis1(i,j,k)=ta1(i,j,k)
!   enddo
!   enddo
!   enddo
